...
func New(service class.ServiceInterface, e *echo.Echo) {
	...
	e.GET("/classes", handler.GetAll)
	e.GET("/classes/:id", handler.GetById)
	e.POST("/classes", handler.Create)
	e.PUT("/classes/:id", handler.Update, middlewares.JWTMiddleware())
	e.DELETE("/classes/:id", handler.Delete, middlewares.JWTMiddleware())
}
...
func (delivery *ClassDelivery) GetById(c echo.Context) error {
	...
	results, err := delivery.classService.GetById(uint(id))
	if err != nil {
		if strings.Contains(err.Error(), "Get data success. No data.") {
			return c.JSON(http.StatusOK, helper.SuccessWithDataResponse(err.Error(), results))
		}
...
func (delivery *ClassDelivery) Update(c echo.Context) error {
	idParam := c.Param("id")
	id, errConv := strconv.Atoi(idParam)
	if errConv != nil {
		return c.JSON(http.StatusBadRequest, helper.FailedResponse("Error. Id must integer."))
	...
	dataCore := toCore(input)
	err := delivery.classService.Update(dataCore, uint(id))
	if err != nil {
		if strings.Contains(err.Error(), "Error:Field validation") {
			return c.JSON(http.StatusBadRequest, helper.FailedResponse("Some field cannot Empty. Details : "+err.Error()))
		}
...
func (delivery *ClassDelivery) Delete(c echo.Context) error {
	idParam := c.Param("id")
	id, errConv := strconv.Atoi(idParam)
	if errConv != nil {
		return c.JSON(http.StatusBadRequest, helper.FailedResponse("Error. Id must integer."))
	}
	err := delivery.classService.Delete(uint(id))
	if err != nil {
		return c.JSON(http.StatusBadRequest, helper.FailedResponse(err.Error()))
	}
